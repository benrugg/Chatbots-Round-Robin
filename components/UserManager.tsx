  ...
  return (
    <div className="space-y-6">
      <div className="flex space-x-2">
        ...
        />
        <button onClick={addTeamMember} className="bg-green-500 hover:bg-green-700 text-white font-bold py-1 px-2 rounded">
          Add Team Member
        </button>
      </div>
      ...
      {archivedMembers.length > 0 && (
        <div className="space-y-4">
          <h2 className="text-xl font-bold text-gray-600">Archived Team Members</h2>
          {archivedMembers.map((member) => (
            <div key={member.id} className="flex items-center justify-between opacity-60">
              <input type="text" value={member.name} className="border rounded px-2 py-1 bg-gray-100" readOnly />
              <button
                onClick={() => toggleArchiveTeamMember(member.id, member.is_archived)}
